(function () {
  ...
  const elements = {
    ...
    physician: document.getElementById("analyticsPhysicianFilter"),
  ...
  function filteredPatients() {
    ...
    return patients.filter((patient) => {
      ...
      const matchesPhysician = elements.physician.value === "all" || patient.physician === elements.physician.value;
      const matchesTreatment = elements.treatment.value === "all" || patient.treatment === elements.treatment.value;
      return matchesFrom && matchesTo && matchesLocation && matchesPhysician && matchesTreatment && matchesAnalyticsStatus(patient);
    });
  }
  ...
  function renderKpis(selected) {
    ...
    const plansNeedingAttention = planSummaries.filter((item) => ["Overdue", "Due Today", "Grace Period"].includes(item.summary.attention));
    const planPastDue = planSummaries.reduce((sum, item) => sum + item.summary.overdueAmount, 0);
    const outcomeCount = selected.reduce((sum, patient) => sum + (patient.outcomes || []).length, 0);
    const followUpCount = selected.filter((patient) => {
    ...
    document.getElementById("outcomeCount").textContent = outcomeCount.toLocaleString();
  ...
  function planStatusPills(patient, summary) {
    const pills = ['<span class="status-pill plan">Payment Plan</span>'];
    if (summary.attention === "Overdue") pills.push(`<span class="status-pill overdue">${summary.overdueCount} overdue</span>`);
    else if (summary.attention === "Due Today") pills.push('<span class="status-pill due">Due today</span>');
    else if (summary.attention === "Grace Period") pills.push('<span class="status-pill grace">Grace period</span>');
    else if (summary.attention === "Plan On Schedule") pills.push('<span class="status-pill current">Plan On Schedule</span>');
    else pills.push('<span class="status-pill paid">Complete</span>');
  ...
  function renderPlanPerformance(selected) {
    const rows = selected
      ...
      .sort((a, b) => {
        const rank = { Overdue: 0, "Due Today": 1, "Grace Period": 2, "Plan On Schedule": 3, Complete: 4 };
        const urgency = (rank[a.summary.attention] ?? 9) - (rank[b.summary.attention] ?? 9);
    ...
    rows.forEach(({ patient, summary }) => {
      ...
      const nextDue = summary.nextDue
        ? `${D.formatDate(summary.nextDue.dueDate)} · ${D.currency.format(summary.nextDue.remaining)}`
        : "Plan complete";
      const terms = [summary.graceDays ? `${summary.graceDays}-day grace` : "No grace", summary.promiseToPayDate ? `Promise ${D.formatDate(summary.promiseToPayDate)}` : "", summary.repeatedMisses ? "Repeated misses" : ""].filter(Boolean).join(" · ");
      const tr = document.createElement("tr");
      tr.innerHTML = `
        ...
        <td data-label="Progress">
          ...
          <div class="progress-track compact-progress"><div class="progress-fill plan-fill" style="width:${summary.progressPercent.toFixed(2)}%"></div></div>
  ...
  function renderInsights(selected) {
    ...
    const overdue = selected.filter((patient) => D.attentionStatus(patient) === "Overdue");
  ...
  [elements.fromDate, elements.toDate, elements.location, elements.physician, elements.treatment, elements.status]
